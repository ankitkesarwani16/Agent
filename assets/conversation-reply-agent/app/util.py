    ...
    for attempt in range(1 + _MCP_RETRY_ATTEMPTS):
        try:
            ...
            try:
                ...
                # If we already captured a result, the call succeeded â suppress teardown noise.
            ...
            if _call_result is None:
                raise RuntimeError(
                    f"call_mcp_tool_with_retry: SDK call_mcp_tool returned None for {mcp_tool.name} â "
